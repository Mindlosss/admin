...
<head>
    ...
    <meta name="description" content="Alta de marcas">
...
<body class="min-h-screen bg-slate-50 text-slate-900" style="font-family: 'Space Grotesk', sans-serif;">
    <div class="min-h-screen bg-[radial-gradient(1200px_circle_at_top,_rgba(244,63,94,0.12),_transparent)]">
        <div class="lg:flex">
            ...
            <div class="flex-1">
                ...
                <main class="mx-auto w-full max-w-6xl px-4 pb-12 pt-6 lg:px-8">
                    <div class="fade-up">
                        <h1 class="text-2xl font-semibold tracking-tight">Agregar marca</h1>
                        <p class="mt-2 text-sm text-slate-600">Carga el nombre y el logo oficial de la marca.</p>
                    </div>
                    ...
                    <div class="mt-6 rounded-2xl border border-slate-200/80 bg-white/80 p-6 shadow-sm">
                        <form method="POST" action="index.php?route=brands" enctype="multipart/form-data" class="space-y-6">
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
                                    <label for="nombre" class="text-xs font-semibold uppercase tracking-widest text-slate-500">Nombre de la marca</label>
                                    <input type="text" id="nombre" name="nombre" class="mt-2 w-full rounded-xl border border-slate-200 bg-white/70 px-4 py-3 text-sm shadow-sm outline-none transition focus:border-sky-400 focus:ring-4 focus:ring-sky-100" placeholder="Ej: Toyota, Honda, Ford" required>
                                </div>
                            </div>

                            <div>
                                <label class="text-xs font-semibold uppercase tracking-widest text-slate-500">Logo de la marca</label>
                                <div id="dropZoneLogo" class="mt-3 flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-slate-300 bg-white/70 px-6 py-8 text-center transition">
                                    ...
                                    <input type="file" id="logo" name="logo" accept=".jpg,.jpeg,.png" class="hidden" required>
    ...
    <script>
        ...
        function renderLogo() {
            ...
            meta.innerHTML = `<span class="truncate" title="${uploadedLogo.name}">${uploadedLogo.name}</span><span>${formatFileSize(uploadedLogo.size)}</span>`;
